...
resource "aws_security_group" "web_server_01_sg" {
  name        = "web-server-01-sg"
  description = "Security group for web-server-01"
...
resource "aws_instance" "web_server_01" {
  ...
  vpc_security_group_ids = [aws_security_group.web_server_01_sg.id]
...
output "web_server_01_instance_id" {
  value       = aws_instance.web_server_01.id
  description = "Instance ID of web-server-01"
}